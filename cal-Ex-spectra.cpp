//This was written by JWB on 13/07/2016 to generate calibrated position spectra for all 24Mg runs from pr254.

{
  ...
  //double gain[109],offset[109];//variables to see what the offset and gain is for each run so that all runs and peaks may be aligned to achieve an optimal resolution in the calibrated focal plane spectrum
  ...
  for(int i=0;i<109;i++)
      ...
      if(runs[i]<2012)
	{
	  if(runs[i]<1056)
	      ...
	      if(runs[i]==1040)
		{
	      ...
	      char histName[256];
	      sprintf(histName,"17.5986-0.0232139*X1posCorr+4.97401e-6*X1posCorr*X1posCorr>>hExRun%d(1000,0,18,1000,0,2000)",runs[i]);
	      ...
	      if(runs[i]==1056)
		{
	      ...
	      char histName[256];
	      sprintf(histName,"17.8034-0.0239224*X1posCorr+5.56636e-6*X1posCorr*X1posCorr>>hExRun%d(1000,0,18,1000,0,2000)",runs[i]);
	      ...
	      if(runs[i]==1076)
		{
	      ...
	      char histName[256];
	      sprintf(histName,"20.2834-0.0290043*X1posCorr+9.35917e-6*X1posCorr*X1posCorr>>hExRun%d(1000,0,18,1000,0,2000)",runs[i]);
	      DATA->Draw(histName,"CUTpad1vstofRun1076 && !X1flag && !X2flag && !U1flag");
	    }//end of if-statement for weekend 1, field-settings 3
	  char cBuffer[256];
	  char pngBuffer[256];

	  sprintf(cBuffer,"/home/wiggert/Documents/pr254/spectra/weekend1/Ex/calExRun%d.C",runs[i]);
	  sprintf(pngBuffer,"/home/wiggert/Documents/pr254/spectra/weekend1/Ex/calExRun%d.png",runs[i]);
      ...
      else if(runs[i]>=2012 && runs[i]<3025)
	{
	  if(runs[i]<2066)
	      ...
	      if(runs[i]==2012)
		{
	      ...
	      char histName[256];
	      sprintf(histName,"19.096-0.0271916*X1posCorr+8.37548e-6*X1posCorr*X1posCorr>>hExRun%d(1000,0,18,1000,0,2000)",runs[i]);
	      ...
	      if(runs[i]==2066)
		{
	      ...
	      char histName[256];
	      sprintf(histName,"19.7331-0.0267545*X1posCorr+7.51906e-6*X1posCorr*X1posCorr>>hExRun%d(1000,0,18,1000,0,2000)",runs[i]);
	  ...
	  sprintf(cBuffer,"/home/wiggert/Documents/pr254/spectra/weekend2/Ex/calExRun%d.C",runs[i]);
	  sprintf(pngBuffer,"/home/wiggert/Documents/pr254/spectra/weekend2/Ex/calExRun%d.png",runs[i]);
      ...
      else
	{
	  ...
	  char histName[256];
	  sprintf(histName,"18.3874-0.0234076*X1posCorr+4.92999e-6*X1posCorr*X1posCorr>>hExRun%d(1000,0,18,1000,0,2000)",runs[i]);
	  DATA->Draw(histName,"CUTpad1vstofRun3025 && !X1flag && !X2flag && !U1flag");

	  char cBuffer[256];
	  char pngBuffer[256];

	  sprintf(cBuffer,"/home/wiggert/Documents/pr254/spectra/weekend3/Ex/calExRun%d.C",runs[i]);
	  sprintf(pngBuffer,"/home/wiggert/Documents/pr254/spectra/weekend3/Ex/calExRun%d.png",runs[i]);
